...
test('getAll gets all todos', () => {
  const expected = 3
  return db.getAll(testDb).then((todos) => {
...
test('addTodo adds a todo', () => {
  return db.addTodo('add a new test task', testDb).then((newIds) => {
...
test('findTodos returns matching todos', () => {
  return db.findTodos('mow', testDb).then((todos) => {
...
test('close destroys the database connection', () => {
  ...
  const query = db.close(testDb).then(() => {
