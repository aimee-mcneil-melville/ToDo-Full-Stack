...
## 4. Client-side: Allow the user to register and log in/out using Auth0
...
1. Call these functions in the two handlers (instead of the `console.log` placeholders):
...
After you've registered your new user, you will be redirected back to `https://localhost:3000`. <ins>This behaviour will change after you complete step 5</ins>.

Commit your code and swap driver/navigator.

## 5. Client-side: Reading user metadata

### I. useEffect in App.jsx:

At this point our app allows users to register/log in and log out using the Auth0 service. There are two places our app needs access to user data generated by Auth0. One of them is in `client/components/App.jsx`. Notice `isAuthenticated`, and `getAccessTokenSilently`, two variables that are currently hard-coded and need to be replaced.

To replace this code, call useAuth0 and destructure:
...
_Note: The `useEffect` hook (in `App.jsx`) will run anytime the user's Auth0 authentication status updates (i.e. when `isAuthenticated` changes) indicating the user has logged in or logged out. If the user is authenticated (logged in) it will use their token to request user information from the database. If user information is received, `dispatch(updateLoggedInUser(userInDb))` will be called and their user information added to the redux global state. If they do not have information present in the database they will be taken to the `/register` url. Our site will now direct us to the /register url until we complete Step 8._
...
_Note: The `cacheUser` function (from `auth0-utils.js`) does `dispatch(updateLoggedInUser(userToSave))`. Every time the `App` component renders, the `cacheUser` function runs, which guarantees that our global state will always have the user's metadata, including their token. We will later pass this token as a header when calling server-side routes that we want to protect._

## 6. Client-side: Passing access tokens

We only want to allow a user to use our server routes if the user has been authenticated. So now the access token is stored in global state (see note above), we want to pass it as a header when calling our server-side routes. In this step, we are going to read `token` and pass it as a parameter to the functions in `api.js` (e.g. when we call the `addFruit` function of `api.js` from the `handleAdd` function of `AddFruit.jsx`).
...
Every time a route receives an HTTP request, the checkJwt middleware (set up in Step 8) will trigger and issue an HTTP request behind the scenes (machine to machine). The Auth0 service will compare the public signatures. If all goes well, `express` will execute the body of your route.
